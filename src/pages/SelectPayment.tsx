...
interface SelectPaymentProps {
  // Definimos onBack en la interfaz por si acaso, pero no lo requerimos obligatoriamente
  onBack?: () => void; 
  onConfirm: () => void;
  price: number;
}

// CORRECCIÓN: Quitamos 'onBack' de aquí abajo para eliminar la advertencia amarilla
export default function SelectPayment({ onConfirm, price }: SelectPaymentProps) {
  const navigate = useNavigate();
  const [selectedId, setSelectedId] = useState<string>('1');

  // Navegación inteligente: Vuelve a la pantalla anterior real en el historial
  const handleBack = () => {
    navigate(-1);
  };
  ...
  return (
    <div className="bg-slate-50 min-h-screen pb-32 font-sans relative">
      ...
      <div className="bg-white sticky top-0 z-20 px-4 py-4 flex items-center shadow-sm">
        <button 
            onClick={handleBack} 
            className="p-2 -ml-2 hover:bg-slate-50 rounded-full transition"
        >
      ...
      <div className="px-6 pt-6">
        ...
        <div className="bg-white p-4 rounded-2xl shadow-sm border border-slate-100 flex items-center gap-4 mb-8">
          ...
          <div className="flex-1">
            <h3 className="font-bold text-slate-900 text-sm">Manicure Gel - At Home</h3>
          ...
          {MOCK_PAYMENT_METHODS.map((method) => {
            ...
            return (
              <div 
                ...
                className={`relative flex items-center gap-4 p-4 rounded-2xl border-2 transition-all cursor-pointer group ${
                  isSelected 
                    ? 'bg-white border-purple-500 shadow-lg shadow-purple-100' 
              ...
              >
                ...
                {/* Checkmark */}
                {isSelected && (
                  <div className="bg-purple-600 rounded-full p-1 text-white animate-fade-in-up">
