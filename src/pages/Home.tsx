...
export const Home = ({ onLogout, onNavigate }: HomeProps) => {
  ...
  const goToCategory = (categoryName: string) => {
    ...
    if (categoryName.includes('Wellness')) filter = 'Massage';
    if (categoryName.includes('Nails')) filter = 'Nails';
    if (categoryName.includes('Facial')) filter = 'Facial';
    if (categoryName.includes('Yoga')) filter = 'Massage';

    navigate('/booking/services', { state: { mode: activeTab, category: filter } });
  };
            ...
            <div className="px-6 mb-8 animate-fade-in">
                ...
                <div className="grid grid-cols-2 gap-3">
                    {[
                        ...
                        { name: 'Spa & Relax', icon: '🌿', bg: 'bg-cyan-100', color: 'text-cyan-600' }, 
                        { name: 'Makeup', icon: '💄', bg: 'bg-pink-100', color: 'text-pink-600' }, 
                        { name: 'Wellness', icon: '🧘‍♀️', bg: 'bg-green-100', color: 'text-green-600' }
                    ].map((cat) => (
                        <button key={cat.name} onClick={() => goToCategory(cat.name)} className="bg-white p-3 rounded-xl border border-slate-100 flex items-center gap-3 hover:bg-slate-50 active:scale-95 transition-all text-left shadow-sm hover:shadow-md">
                ...
                <div className="mb-8">
                    ...
                    <div className="flex justify-between">
                        {[
                            ...
                            <button key={s.name} onClick={() => goToCategory(s.name)} className="flex flex-col items-center gap-2 group cursor-pointer active:scale-90 transition-transform">
